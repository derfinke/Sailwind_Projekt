...
int main(void) {
	...
	MX_TIM3_Init();
	/* USER CODE BEGIN 2 */
	linear_guide = Linear_Guide_init(&hdac, &htim3, TIM_CHANNEL_4,
...
//#endif
  ...
  printf("read from FRAM: %u\r\n", test);
	...
	uint8_t calib_started = 0;
	while (1) {
#if !TEST
		while (calib_started == 0) {
			if (Button_state_changed(&manual_control.buttons.localize)) {
				calib_started = 1;
			}
		}
//		WSWD_receive_NMEA(NMEA);
...
//		printf("speed:%f, dir:%f\r\n", speed, dir);
		...
		if (manual_control.lg_ptr->localization.is_localized) {
			if (manual_control.lg_ptr->sail_adjustment_mode
					== LG_sail_adjustment_mode_rollung) {
				LED_switch(&manual_control.lg_ptr->leds.rollung, LED_ON);
				LED_switch(&manual_control.lg_ptr->leds.trimmung, LED_OFF);
			} else if (manual_control.lg_ptr->sail_adjustment_mode
					== LG_sail_adjustment_mode_trimmung) {
				LED_switch(&manual_control.lg_ptr->leds.trimmung, LED_ON);
				LED_switch(&manual_control.lg_ptr->leds.rollung, LED_OFF);
			}
		}
#else
...
static void MX_GPIO_Init(void) {
	...
	HAL_GPIO_WritePin(GPIOB,
			LED_Automatik_Pin | Ext_Relais_2_Pin | LED_Trimmen_Pin | IN_0_Pin,
			GPIO_PIN_RESET);
	...
	HAL_GPIO_WritePin(GPIOD,
			LED_Kalibrieren_Speichern_Pin | LED_Stoerung_Pin | LED_Manuell_Pin
					| IN_2_Pin | Windsensor_EN_Pin, GPIO_PIN_RESET);
	...
	GPIO_InitStruct.Pin = Switch_Betriebsmodus_Pin
			| Betriebsmodus_2_Nur_Prototyp_Pin | Betriebsmodus_Prototyp_Pin;
	...
	GPIO_InitStruct.Pin = LED_Automatik_Pin | Ext_Relais_2_Pin | LED_Trimmen_Pin
			| LED_PWR_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
	...
	GPIO_InitStruct.Pin = LED_Kalibrieren_Speichern_Pin | LED_Stoerung_Pin
			| LED_Manuell_Pin | Windsensor_EN_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
	HAL_GPIO_Init(GPIOD, &GPIO_InitStruct);

	/*Configure GPIO pins : IN_1_Pin IN_3_Pin */
	GPIO_InitStruct.Pin = IN_1_Pin | IN_3_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Pull = GPIO_PULLDOWN;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
	HAL_GPIO_Init(GPIOC, &GPIO_InitStruct);
	...
	/*Configure GPIO pin : IN_0_Pin */
	GPIO_InitStruct.Pin = IN_0_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Pull = GPIO_PULLDOWN;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
