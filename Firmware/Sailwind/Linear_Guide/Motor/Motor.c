 ...
 */
void Motor_teach_speed(Motor_t *motor_ptr, Motor_function_t speed,
                       uint16_t rpm_value, uint32_t tolerance) {
  printf("\nstoppt Motor...\n");
  Motor_set_function(motor_ptr, Motor_function_stop_holding_torque);
  printf(
      "Enter drücken um Lernmodus einzuleiten, sobald Motor angehalten...\n");
  ...
  printf(
      "Lernmodus aktiviert, wenn rote LED schnell blinkt -> Enter um fortzufahren...\n");
  Motor_press_enter_to_continue();
  ...
  Motor_set_function(motor_ptr, Motor_function_stop_holding_torque);
  printf(
      "Enter drücken um Lernmodus zu verlassen, sobald Motor angehalten...\n");
  Motor_press_enter_to_continue();
  ...
  printf(
      "Lernmodus deaktiviert, wenn rote LED langsam blinkt -> Enter um fortzufahren...\n");
  Motor_press_enter_to_continue();
}
...
static Motor_RPM_Measurement_t Motor_OUT1_init(
    TIM_HandleTypeDef *htim_ptr, uint32_t htim_channel,
    HAL_TIM_ActiveChannel htim_active_channel) {
  Motor_RPM_Measurement_t rpm_measurement = { .IC_Val1 = 0, .IC_Val2 = 0,
      .Is_First_Captured = False, .htim_ptr = htim_ptr, .htim_channel =
          htim_channel, .active_channel = htim_active_channel, .rpm_value = 0 };
  return rpm_measurement;
}

static IO_analogActuator_t Motor_AIN_init(DAC_HandleTypeDef *hdac_ptr) {
  IO_analogActuator_t rpm_setting = { .hdac_ptr = hdac_ptr, .hdac_channel =
      DAC_CHANNEL_1, .maxConvertedValue = MOTOR_RPM_MAX, .limitConvertedValue =
      MOTOR_RPM_LIMIT, .currentConvertedValue = 0.0F, .dac_value = 0 };
  return rpm_setting;
}
