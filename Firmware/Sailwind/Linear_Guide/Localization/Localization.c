...
void Loc_set_endpos(Localization_t *loc_ptr)
{
	loc_ptr->end_pos_mm = Loc_convert_pulse_count_to_distance(*loc_ptr)/2;
...
void Loc_update_pulse_count(Localization_t *loc_ptr)
{
	...
	{
		Loc_update_current_position(loc_ptr);
	}
}
...
void Loc_update_current_position(Localization_t *loc_ptr)
{
	int32_t absolute_distance = Loc_convert_pulse_count_to_distance(*loc_ptr);
